...
library(doParallel)
library(iterators)
#data(M3)  # load M3 data
#devtools::load_all(".")  # load local forecTheta
...
out_dotm <- foreach(i = 1:3003, 
               .packages = c("foreach", "forecast", "Mcomp")) %dopar% {
                 x <- M3[[i]]$x
                 h <- M3[[i]]$h
...
for (mo in 1:length(out)) {
  ...
  #### ALL 2 ###
  #sAPE_matrix %>% rowMeans(na.rm=T) %>% mean()
  ...
  #### ALL 2 ###
  #ASE_matrix %>% rowMeans(na.rm=T) %>% mean()
